...
import { Shield, Users, Globe, Lock, Crown, BarChart3, ArrowLeft, Loader2, Trash2, ClipboardPaste } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.message || 'Unauthorized access attempt logged.');
      }
    } catch {
      alert('Login error');
    } finally {
  ...
  const handlePurgeAll = async () => {
    if (!confirm('☢️ CRITICAL ACTION: This will delete ALL rooms globally. Are you 100% sure?')) return;
    try {
      const res = await fetch('/api/admin/manage', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
    ...
    return (
      <div className="min-h-screen bg-[#050505] text-white flex items-center justify-center p-6">
        ...
        >
          ...
          <button disabled={loading} className="w-full bg-white text-black font-black py-5 rounded-2xl hover:scale-[1.02] active:scale-95 transition-all text-sm uppercase tracking-widest disabled:opacity-50 flex items-center justify-center">
            {loading ? <Loader2 className="animate-spin w-5 h-5" /> : 'Access Control'}
          </button>
        </motion.form>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#050505] text-white p-6 md:p-12 font-sans selection:bg-blue-500/30">
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard icon={<Globe className="text-blue-500"/>} label="Frank Link" value={stats?.totalRooms || '0'} sub="P2P Clusters" />
          <StatCard icon={<BarChart3 className="text-purple-500"/>} label="Frank Notes" value={stats?.totalNotebooks || '0'} sub="Cloud Logs" />
          <StatCard icon={<Crown className="text-amber-500"/>} label="Frank Nearby" value={stats?.totalNearby || '0'} sub="Local Networks" />
           ...
           <div className="lg:col-span-2 space-y-6">
              <div className="flex items-center justify-between bg-white/[0.03] p-2 rounded-3xl border border-white/5">
                 {[
                   ...
                   { id: 'nearby', label: 'Nearby', icon: ClipboardPaste }
                 ].map(eco => (
                   <button 
                     key={eco.id}
                      ...
                      items.map((item, idx) => (
                        <div key={idx} className="group p-5 bg-black/40 border border-white/5 rounded-2xl flex items-center justify-between hover:border-blue-500/30 transition-all">
                           <div className="flex items-center gap-4">
                              ...
                              <div>
                                 <div className="text-sm font-bold truncate max-w-[200px]">{item.roomId || item.id}</div>
                                 <div className="text-[10px] text-gray-500 font-mono italic">{new Date(item.createdAt || item.joinedAt).toLocaleDateString()}</div>
                              </div>
                           </div>
                           <div className="flex items-center gap-4">
                              ...
                              <button onClick={() => handleDelete(item.roomId || item.id)} className="p-3 bg-red-500/10 rounded-xl hover:bg-red-500/20 transition-all">
