...
export function useFrankRTC(roomId: string) {
  ...
  const setupDataChannel = (channel: RTCDataChannel) => {
    ...
    channel.onmessage = (event) => {
      // Stub for Phase 3 completion logic. We use useFileTransfer for complex chunk handling.
      if (typeof event.data === 'string') {
        const textData = JSON.parse(event.data);
        if (textData.type === 'chat') {
            setMessages(prev => [...prev, { id: Math.random().toString(), sender: 'them', type: 'text', content: textData.text, timestamp: new Date() }]);
        }
      }
    };
  };
  ...
  return { isConnected, messages, sendMessage, channelRef, dataChannelRef, peerRef };
}
